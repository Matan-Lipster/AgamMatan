...
def process_data(test, subject_id):
    ...
    if test == "Default_PFC":
        ...
        subset_df = subject_row[["PMAT24_A_CR"]]
        all_labels = df_7T[["PMAT24_A_CR"]].values.astype(np.float32)
    else:
...
def process_directory(subject_folder, H, NET, NET_idx, Avg, noise_level=0.01, n_synthetic_TRs=3):
    try:
        if H == 'BOTH':
            ...
            data_files = glob.glob(osp.join(subject_folder, H + '' + NET + '' + str(NET_idx) + '.pkl'))
        if not data_files:
        ...
        if H == 'BOTH':
            ...
            output_file = osp.join(subject_folder, H + '' + NET + '' + str(NET_idx) + '_Avg.pkl')
...
def create_synthetic_subjects(subject_folder, H, NET, NET_idx, noise_level=0.01, n_synthetic_subjects=10):
    ...
    for i in range(n_synthetic_subjects):
        ...
        synthetic_folder = f"{subject_folder}SYNTH{i + 1}"
        ...
        if H == 'BOTH':
            ...
            files = glob.glob(osp.join(subject_folder, H + '' + NET + '' + str(NET_idx) + '.pkl'))
    ...
    for phase in ['train', 'eval', 'test'] :
        ...
        for subject_folder in subject_folders:
            ...
            try:
                if Avg in [7, 2]:
                    if H == "BOTH":
                        file_path = osp.join(subject_folder, '{}{}{}.pkl'.format(H, NET, 'Avg'))
                    else:
                        file_path = osp.join(subject_folder, '{}{}{}_{}.pkl'.format(H, NET, NET_idx, 'Avg'))
                else:
                    file_path = osp.join(subject_folder, '{}{}{}.pkl'.format(H, NET, NET_idx))
    ...
    if Group_by_subjects == 1:
        ...
        for i in range(0, len(subject_folders), group_size):
            if len(subject_folders[i:i + group_size]) < group_size:
                break
            group_folders = subject_folders[i:i + group_size]
            group_folder_name = f"GROUP_{osp.basename(group_folders[0])}{osp.basename(group_folders[-1])}{run_suffix}"
...
def get_dataloaders2_random_split(directory, NET, NET_idx, slice, batch_size, df_7T, Avg=0, noise_level=0.01,
    ...
    for subject_folder in subject_folders:
        try:
            movie_files = glob.glob(os.path.join(subject_folder, 'movie_*.pkl'))
            subject_movies = []
            ...
            for movie_file in movie_files:
                try:
                    ...
                    if not os.path.exists(movie_file):
                        print(f"File does not exist: {movie_file}")
                        continue

                    if Avg == 1:
                        data_vis = average_voxels([movie_file], n_components=20)
                    else:
                        with open(movie_file, 'rb') as file:
                            data_vis = pickle.load(file)

                    subject_movies.append(torch.tensor(data_vis, dtype=torch.float64))
                    counter += 1

                except Exception as e:
                    print(f"Error processing file {movie_file} in subject {subject_folder}: {e}")
                    continue

            if not subject_movies:
            ...
            print(f"Error processing subject folder {subject_folder}: {e}")
            continue

    if len(inputs) == 0 or len(outputs) == 0:
        return None, None, None, None, None, None

    # Stack inputs and outputs into tensors
    tensor_inputs = torch.stack(inputs)
    tensor_outputs = torch.stack(outputs)
    ...
    train_size = round(split_ratio[0] * len(dataset))
    val_size = round(split_ratio[1] * len(dataset))
    test_size = len(dataset) - train_size - val_size
